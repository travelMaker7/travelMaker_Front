...
describe("<JoinRequestButton />", () => {
  ...
  it('should render button with text "동행신청"', () => {
    render(
      <JoinRequestButton tripPlanId={1} isVisible={true} overWish={false} />
    );
    const button = screen.getByText("동행신청");
    expect(button).toBeInTheDocument();
  });

  it('should change button text to "동행신청" after click', async () => {
    mock.onPost("/api/v1/accompany/guest").reply(200, {
      message: "동행 신청이 완료되었습니다.",
    });

    render(
      <JoinRequestButton tripPlanId={1} isVisible={true} overWish={true} />
    );
    const button = screen.getByText("신청불가");
