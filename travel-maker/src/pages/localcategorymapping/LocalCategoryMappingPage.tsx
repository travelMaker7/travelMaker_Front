...
const LocalCategoryMappingPage: React.FC = () => {
  const [searchResults, setSearchResults] = useState<Schedule[]>([]);
  ...
  //   console.log(searchResults);
  // };
  const handleMarkerClick = async (markerData: { destinationX: string; destinationY: string; }) => {
    setLoading(true);
    try {
        const response = await axios.get<{ data: { schedules: Schedule[] } }>(`https://sosak.store/api/v1/trip/search/?page=1&size=10&destinationX=&destinationY=`, {
            params: {
                destinationX: markerData.destinationX,
                destinationY: markerData.destinationY,
                page: 0,  // 페이지와 사이즈는 예시 값
                size: 4
            }
        });
        setSearchResults(response.data.data.schedules);
        console.log("보여줘:",response.data.data);
    } catch (error) {
  ...
  return (
    ...
    <>
      ...
      <MainContainer>
        <MapContainer>
        <LocalMap onMarkerClick={handleMarkerClick} />
        </MapContainer>
        {loading ? <p>Loading...</p> : (
        <ListContainer
          spaceBetween={50}
